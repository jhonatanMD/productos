...
public class ControllerConsultaVoz {
    ...
    Maybe<?> consulta(@RequestBody @Valid ConsultasVoz.consulta request) {
        ...
        if(consulta.indexOf(Constantes.CONSULTA_PRODUCTO_1) == 0) {
            ...
            return consultaVozService.productos(producto)
            ...
            return consultaVozService.productos(producto).map(res -> new ProductoConsulta().new RespuestaConsultaProducto("Se encontro lo siguiente de "+producto,res))
                    .switchIfEmpty(Maybe.just(new ProductoConsulta().new RespuestaConsultaProducto("No se encontro producto "+producto, Arrays.asList())));
